<?php
/**
 * admin_reports.php — 举报管理后台
 *
 * 功能：展示待审核举报，支持快速删帖/封号/驳回/标记已处理
 * 读写表：读写 reports、posts、users
 * 权限：admin / owner
 */
...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .ftab { padding: 5px 14px; border-radius: 20px; font-size: 12px; text-decoration: none; border: 1px solid 
                background: 
        .ftab:hover { border-color: 
        .ftab.active { background: rgba(240,136,62,.15); color: 
        .ftab .n { display: inline-block; background: 
                   font-size: 10px; margin-left: 4px; color: 
        .ftab.active .n { background: rgba(240,136,62,.2); color: 
        .filter-sep { width: 1px; height: 20px; background: 
        ...
        .badge-post { background: rgba(88,166,255,.12); color: 
        .badge-user { background: rgba(167,139,250,.12); color: 

        .rc-body { flex: 1; min-width: 0; }
        .rc-reason { font-size: 14px; font-weight: 700; color: 
        .rc-detail { font-size: 12px; color: 
        .rc-meta { display: flex; flex-wrap: wrap; gap: 10px; font-size: 11px; color: 
        ...
        .status-tag { padding: 2px 8px; border-radius: 3px; font-size: 11px; font-weight: 700;
                      font-family: "Courier New", monospace; }
        .st-pending   { background: rgba(240,136,62,.12); color: 
        .st-handled   { background: rgba(63,185,80,.1);  color: 
        .st-dismissed { background: rgba(110,118,129,.1); color: 
