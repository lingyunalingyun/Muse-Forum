 ...
 *
 * 功能：向客户端发送 503 Service Unavailable 状态码并展示暗色主题服务暂不可用提示页面。
 * 读写表：无
 * 权限：无
 */
...
.progress-label{font-size:11px;color:
.progress-label span{color:var(--c);}
.bar-wrap{background:
.bar{height:100%;width:73%;background:linear-gradient(90deg,
...
.task{font-size:12px;display:flex;align-items:center;gap:8px;color:
.task .ok{color:var(--c);}.task .spin{animation:spin2 1s linear infinite;display:inline-block;color:
